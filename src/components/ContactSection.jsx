import React from 'react'
import ContactForm from './ContactForm';


const contactsArray = [
    {
        category: 'Office',
        emails: [
            'info@tolobsmarinegrp.com ',
            'info@tolobsmarinegrp.com ',
            'legal@tolobsmarinegrp.com ',
            'accounts@tolobsmarinegrp.com '
        ]
    },
    {
        category: 'Operation',
        emails: [
            'opsphc@tolobsmarinegrp.com ',
            'opsphc@tolobsmarinegrp.com ',
            'opswarri@tolobsmarinegrp.com ',
            'opswarri@tolobsmarinegrp.com ',
            'opscalabar@tolobsmarinegrp.com ',
            'opslagos@tolobsmarinegrp.com ',
            'opsabuja@tolobsmarinegrp.com ',
            'opsbonny@tolobsmarinegrp.com '
        ]
    },
    {
        category: 'Contact',
        emails: [
            'osaki-braide@tolobsmarinegrp.com',
            'alafa-a.romeo@tolobsmarinegrp.com ',
            'silas.etere@tolobsmarinegrp.com'
        ]
    }
];

const ContactSection = () => {
    return (
        <div className='relative '>

        <div data-aos="fade-up" className=' flex justify-between buttongradient text-neutral-100 mx-96 max-md:mx-8 max-lg:mx-16 max-2xl:mx-32 max-sm:mx-4 p-4  max-xl:mx-16 max-md:flex-col gap-y-8 gap-x-8 pb-48 '>
            <div className='flex flex-col gap-4 '>
                <div>
                    <h1 className=' text-4xl max-md:text-2xl max-sm:text-xl title w-fit font-semibold'>
                        CONTACT US
                    </h1>
                </div>
                <div className='text-xl max-sm:text-base flex flex-col gap-4'>
                    <div className=' flex flex-col gap-8'>
                        <address className=' not-italic text-neutral-300'>
                            <h1 className=' font-semibold text-neutral-400'>Head Office</h1>
                            <p>198 Bank Anthony Avenue</p>
                            <p>Trans-Amadi Industrial Layout</p>
                            <p>Port Harcourt</p>
                            <p>Rivers State</p>
                            <p>Nigeria</p>
                            <p>Tel: +234 (0)80 3304 0181 / +234 (0)80 3461 9286</p>
                            <p>Email: <a href="mailto:opsphc@tolobsmarinegrp.com ">opsphc@tolobsmarinegrp.com </a></p>
                            <p>Inquiry: <a href="mailto:info@tolobsmarinegrp.com ">info@tolobsmarinegrp.com </a></p>
                        </address>
                        <address className=' not-italic text-neutral-300'>
                            <h1 className=' font-semibold text-neutral-400'>Lagos Office</h1>
                            <p>1, Commercial Road,</p>
                            <p>1st Floor Left Wing, Aries Block, Eleganza Plaza,</p>
                            <p>Apapa Lagos- Nigeria.</p>
                            <p>Tel: +234 (0)90 5190 3878</p>
                            <p>Email: <a href="mailto:opslagos@tolobsmarinegrp.com ">opslagos@tolobsmarinegrp.com </a></p>
                        </address>
                        <div className=' flex flex-col gap-4'>
                            {
                                contactsArray.map((item) => {
                                    return (
                                        <div>
                                            <h1 className='font-semibold text-neutral-400'>{item.category} E-mails</h1>
                                            {
                                                item.emails.map((arrayItem) => {
                                                    return (
                                                        <a href={`mailto:${arrayItem}`} className=' block cursor-pointer'>{arrayItem}</a>
                                                    )
                                                })
                                            }
                                        </div>
                                    )
                                })
                            }
                        </div>
                    </div>
                </div>
            </div>
            <ContactForm />
        </div>
            <div className="custom-shape-divider-bottom-1704712485">
                <svg data-name="Layer 1" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 1200 120" preserveAspectRatio="none" style={{ zIndex: 0 }}>
                    <path d="M0,0V46.29c47.79,22.2,103.59,32.17,158,28,70.36-5.37,136.33-33.31,206.8-37.5C438.64,32.43,512.34,53.67,583,72.05c69.27,18,138.3,24.88,209.4,13.08,36.15-6,69.85-17.84,104.45-29.34C989.49,25,1113-14.29,1200,52.47V0Z" opacity=".25" class="shape-fill"></path>
                    <path d="M0,0V15.81C13,36.92,27.64,56.86,47.69,72.05,99.41,111.27,165,111,224.58,91.58c31.15-10.15,60.09-26.07,89.67-39.8,40.92-19,84.73-46,130.83-49.67,36.26-2.85,70.9,9.42,98.6,31.56,31.77,25.39,62.32,62,103.63,73,40.44,10.79,81.35-6.69,119.13-24.28s75.16-39,116.92-43.05c59.73-5.85,113.28,22.88,168.9,38.84,30.2,8.66,59,6.17,87.09-7.5,22.43-10.89,48-26.93,60.65-49.24V0Z" opacity=".5" class="shape-fill"></path>
                    <path d="M0,0V5.63C149.93,59,314.09,71.32,475.83,42.57c43-7.64,84.23-20.12,127.61-26.46,59-8.63,112.48,12.24,165.56,35.4C827.93,77.22,886,95.24,951.2,90c86.53-7,172.46-45.71,248.8-84.81V0Z" class="shape-fill"></path>
                </svg>
            </div>
        </div>
    )
}

export default ContactSection